...
cw = 640,
    ...
    Particle = function(_i) {
      this.index = _i;
      this.draw = function() {
        bEffect.globalAlpha = this.alpha;
        bEffect.globalCompositeOperation = 'lighter';
        // if (particleNumber%5==0) ctx.drawImage(img, this.x, this.y, this.size, this.size*1.1);
        if (particleNumber%this.index==0) bEffect.drawImage(img, this.x, this.y-2, this.size, this.size*1.3);
        bEffect.drawImage(img, this.x, this.y, this.size, this.size);
      }
    };
...
TweenMax.ticker.addEventListener("tick", function(){
  if(specialCardSwitchFlag == 0 && (specialCardFlag == 1 || btnFlag == 0)){
